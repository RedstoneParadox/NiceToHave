...
open class OldConfigOption<T : Any>(val type : Class<T>, var value : T, val key : String, val comment : String) {
    ...
    protected fun primitiveToValue(any : Any): T {
        if (any::class.java != type) {
            NiceToHave.error("Expected type of `${OldConfig.classToType(type)}` for config option `$key` but instead found type of `${OldConfig.classToType(any::class.java)}`. Will use default value of `$value`.")
            return value
        }
        return any as T
    }
