...
class OldConfigCategory(val key : String? = null, val comment : String? = null, val isMain : Boolean = false) {

    private val optionsMap : HashMap<String, OldConfigOption<*>> = HashMap()
    private val subCategoriesMap : HashMap<String, OldConfigCategory> = HashMap()
    ...
    fun deserialize(json : JsonObject) {
        for (entry in json) {
            if (entry.value is JsonPrimitive) {
                if (optionsMap[entry.key] is OldConfigOption<*>) {
    ...
    fun addOption(optionKey : String, option: OldConfigOption<*>) {
        optionsMap[optionKey] = option
    }

    fun addSubCategory(categoryKey : String, categoryOld: OldConfigCategory) {
        subCategoriesMap[categoryKey] = categoryOld
    }
