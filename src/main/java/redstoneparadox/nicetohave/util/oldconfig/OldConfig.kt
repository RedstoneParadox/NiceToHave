...
@Deprecated("Moving to new system.")
object OldConfig {

    private val MAIN_CATEGORY_OLD : OldConfigCategory
    ...
    init {
        ...
        MAIN_CATEGORY_OLD = Builder(configObject)
                ...
                .addBool("glue_slabs", "Allows for slabs to be glued back together with slimeballs.")
                .endCategory()
    ...
    private fun <T : Any> getOption(key: String, default : T, optionType : Class<T>): T {
        ...
        return MAIN_CATEGORY_OLD.getOption(keySequence, default, key, optionType)
    }

    private fun save() {
        if (!hadError) {
            val configString = MAIN_CATEGORY_OLD.serialize(JsonObject()).toJson(true, true)
    ...
    class Builder(val json : JsonObject) {
        private val mainCategoryOld : OldConfigCategory = OldConfigCategory(isMain = true)
        private var currentCategoryOld : OldConfigCategory = mainCategoryOld

        fun newCategory(key: String, comment : String): Builder {
            currentCategoryOld = OldConfigCategory(key, comment)
            mainCategoryOld.addSubCategory(key, currentCategoryOld)
            return this
        }

        fun endCategory(): Builder {
            currentCategoryOld = mainCategoryOld
            return this
        }

        fun addBool(key: String, comment: String, default : Boolean = true): Builder {
            currentCategoryOld.addOption(key, OldConfigOption(boolType, default, trueKey(key), "$comment [True/False]"))
            return this
        }

        fun addDouble(key: String, comment: String, default : Double): Builder {
            currentCategoryOld.addOption(key, OldConfigOption(doubleType, default, trueKey(key), "$comment [Any Real Number]"))
            return this
        }

        fun addRange(key: String, comment: String, default: Double, min : Double, max : Double): Builder {
            currentCategoryOld.addOption(key, RangeOldConfigOption(default, min, max, trueKey(key), "$comment [Range: $min to $max]"))
            return this
        }

        fun addMetaInt(key: String, comment: String, value : Int): Builder {
            mainCategoryOld.addOption(key, OldConfigMetaData(intType, value, key, "$comment [Meta Data]"))
            return this
        }

        fun build(): OldConfigCategory {
            mainCategoryOld.deserialize(json)
            return mainCategoryOld
        }

        private fun trueKey(key : String): String {
            if (currentCategoryOld == mainCategoryOld) {
                return key
            }
            return "${currentCategoryOld.key}.$key"
        }
    }
